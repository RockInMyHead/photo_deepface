...
# --- Robust OpenCV I/O for Windows Unicode paths ---
# OpenCV's imread/imwrite may fail on non-ASCII Windows paths. Use fromfile/tofile workaround.
def _cv2_imread_unicode(p):
    if cv2 is None:
        return None
    try:
...
def _cv2_imwrite_unicode(p, img, params=None):
    if cv2 is None:
        return False
    try:
        ext = '.' + str(p).split('.')[-1]
        ret, buf = cv2.imencode(ext, img, params if params is not None else [])
        if not ret:
            return False
        buf.tofile(str(p))
        return True
...
def detect_faces_haar(image_path: Path) -> List[Tuple[int, int, int, int]]:
    ...
    if cv2 is None:
        return []
    try:
        im = _cv2_imread_unicode(image_path)
        if im is None:
            return []
