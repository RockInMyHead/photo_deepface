# app.py — Single Explorer kept (with folder previews), no duplicate explorers
# Требования пользователя:
# - Оставить один проводник (тот, что мы считаем «DnD-эксплорером»), второй удалить.
# - В этом проводнике добавить превью для ПАПОК.
# - Не дублировать папки (второй список/проводник удалён), фотографии показывать превью, а не путь.
# - Остальной функционал сохранить (очередь, обработка, rename/delete, LAN URL, история).
...
st.set_page_config(page_title="Face Sorter — единый проводник с превью папок", layout="wide")
...
def render_single_explorer(curr: Path, parent_root: Path):
    """Единый проводник (тот самый div[7]), с DnD по файлам и превью папок/фото."""
    if sort_items is None:
        st.error("DnD требует пакет: pip install streamlit-sortables")
        return

    # Источники DnD — ТОЛЬКО ФАЙЛЫ текущей папки (не дублируем папки)
    ...
    # Для DnD используем имена файлов (не пути) + маппинг name->path (имён дубликатов в папке быть не может)
    file_names = [f.name for f in files_in_curr]
    name_to_path = {f.name: f for f in files_in_curr}

    # Приёмники: вверх + подпапки
    subfolders = folders_in_curr
    up_label = None
    if curr != parent_root:
        up_disp = curr.parent.name or str(curr.parent)
        up_label = f"⬆️ Вверх ({up_disp})"

    containers = [{"header": "📦 Файлы (перетащите на папку ниже)", "items": file_names}]
    ...
    # Добавляем подпапки как цели. В header рендерим только текст, без дубликатов в источниках.
    for f in subfolders:
    ...
    # Панель превью (в этом же проводнике): папки с обложкой + файлы с превью
    ...
    with st.container(height=600):
        # Папки — сверху, файлы — ниже (в одном проводнике)
        items = folders_in_curr + files_in_curr
        for item in items:
            is_dir = item.is_dir()
    ...
    # Применение DnD — двигаем ФАЙЛЫ в выбранные папки/вверх, без дубликатов
    if st.button("Применить перемещения", key=_k("apply_dnd", str(curr)), type="primary", use_container_width=True):
        ...
        ok = skp = err = 0
        first_container = next((c for c in result if c.get("header") == "📦 Файлы (перетащите на папку ниже)"), None)
        original_files = set(file_names)
        for cont in (result or []):
            header = cont.get("header", "")
            dst_dir = header_to_dir.get(header)
            if not dst_dir:
                continue
            for fname in cont.get("items", []):
                ...
                try:
                    ...
                    st.error(f"Ошибка перемещения {fname}: {e}")
                    err += 1
...
st.title("Face Sorter — единый проводник (превью папок + DnD файлов)")
...
if st.session_state["parent_path"] is None:
    ...
    # Breadcrumbs within root (визуальная навигация остаётся)
    ...
    # === Единственный проводник (тот, что "div[7]") ===
