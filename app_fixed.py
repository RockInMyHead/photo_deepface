# app_fixed.py — Face Sorter — risk‑mitigated version
# Fixes for issues 1..11:
# 1) Safe source-bound deletions + feature flag + audit log
# 2) Env checks (numpy/Pillow/streamlit-sortables/send2trash) with UI gating
# 3) DnD idempotency (first-assignment wins) and dedupe
# 4) Cross‑platform folder picking fallback
# 5) Config fields wired into build_plan
# 6) Sorting controls (name/date/size, asc/desc)
# 7) Stable hashed widget keys
# 8) Robust LAN IP candidates + selector
# 9) File lock for index writes (if filelock installed)
# 10) Safer deletion without recycle bin (double confirm)
# 11) Run history tracked per execution
...
st.set_page_config(page_title="Face Sorter — Мини-проводник (safe)", layout="wide")
...
def get_lan_candidates() -> List[str]:
    """Return possible LAN IPs, best effort without extra deps."""
    cand = set()
    # UDP trick
    try:
...
# ---------- DnD (compact, idempotent) ----------

def render_compact_dnd(curr: Path, parent_root: Path):
    if sort_items is None:
        st.info("DnD недоступен: pip install streamlit-sortables")
        return

    # Sources — folders + images
    src_items = []
    ...
    if st.button("Перенести", key="dnd_apply", type="primary", use_container_width=True):
        ...
        # Idempotent assignment: first receiver wins
...
if st.session_state["parent_path"] is None:
    ...
    # Header row (CSS kept minimal inline)
                ...
                for k, gdir in enumerate(targets, start=1):
                    ...
                    try:
                        ...
                        if st.session_state.get("del_after_copy", False):
                            # Only delete sources that still exist and are regular files, outside protected zones
