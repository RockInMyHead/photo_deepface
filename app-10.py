# app.py — Face Sorter (all objects + folder cover thumbs + per-file DnD)
#
# Что внутри:
# - Проводник выводит **и папки, и файлы**. Папки показывают **обложку** —
#   миниатюру первого изображения внутри папки (не рекурсивно).
# - DnD для **каждого файла**: можно менять порядок файлов в текущей папке (UI-порядок)
#   и перетаскивать файл на любую папку (включая «Вверх» и дерево под root) ⇒ физический move с защитой от коллизий.
# - Очередь групп (папок), обработка через `build_plan`, безопасные операции и история прогонов.
...
st.set_page_config(page_title="Face Sorter — проводник с обложками и DnD файлов", layout="wide")
...
def _init_state():
    ...
    st.session_state.setdefault("file_order", {})  # folder->list[str]
...
# ---------- DnD: per-file ordering + move to any folder ----------

def _iter_tree_dirs(root: Path, max_depth: int = 5, max_nodes: int = 500) -> List[Path]:
...
def apply_session_order(curr: Path, items: List[Path]) -> List[Path]:
    order_map = {p: i for i, p in enumerate(st.session_state.get('file_order', {}).get(str(curr), []))}
    def _key(x: Path):
        primary = 0 if x.is_dir() else 1
        if x.is_dir():
            return (primary, x.name.lower())
        return (primary, order_map.get(str(x), 10**9), x.name.lower())
    return sorted(items, key=_key)


def render_dnd_files_any_folder(curr: Path, parent_root: Path, *, max_depth: int = 5, max_nodes: int = 500):
    if sort_items is None:
        st.error("DnD требует пакет: pip install streamlit-sortables")
        return

    # Sources: files in current folder
    files: List[str] = []
    try:
    ...
    # Targets: Up, immediate subfolders, and tree under root
    ...
    tree_dirs = _iter_tree_dirs(parent_root, max_depth=max_depth, max_nodes=max_nodes)

    containers: List[Dict] = []
    containers.append({"header": "📦 Файлы текущей папки (меняйте порядок или перетаскивайте на папки ниже)", "items": files})
    ...
    for f in subfolders:
        h = f"📁 {f.name}"
        containers.append({"header": h, "items": []})
        header_to_dir[h] = f

    for d in tree_dirs:
        if d == curr or d == curr.parent:
            continue
        try:
            rel = d.relative_to(parent_root).as_posix()
            h = f"🗀 {rel}"
        except Exception:
            h = f"🗀 {d.name}"
        containers.append({"header": h, "items": []})
    ...
    if st.button("Применить изменения", key=_k("apply_dnd", str(curr)), type="primary", use_container_width=True):
        if not result:
            st.info("Нет изменений."); return

        # Persist order of files from the first container
        first = next((c for c in result if c.get('header') == "📦 Файлы текущей папки (меняйте порядок или перетаскивайте на папки ниже)"), None)
        if first:
            ordered = [s for s in first.get('items', []) if Path(s).parent.resolve() == curr.resolve()]
            st.session_state.setdefault('file_order', {})[str(curr)] = ordered

        # Apply physical moves for files dropped to folder targets
        ok = skp = err = 0
        files_set = set(files)
        for cont in result:
            ...
            for src_str in cont.get('items', []):
                ...
                try:
                    ...
                    if src.parent.resolve() == dst_dir.resolve():
                        skp += 1; continue
                    # guard: avoid nonsensical subpath checks for files
...
# ---------- Explorer grid (all objects + folder covers) ----------

def render_explorer_grid(curr: Path, parent_root: Path):
    ...
    # Respect session order for files
    items = apply_session_order(curr, items)
    ...
    with st.container(height=700):
        for item in items:
            ...
            with c1:
                if is_dir:
                    try:
                        ...
                        mtime = 0.0
                    data = folder_cover_bytes(str(item), 150, mtime)
                    if data:
                        st.image(data)
                    else:
            ...
            with c2:
                ...
                with name_cols[1]:
                    if is_dir:
                        checked = st.checkbox("Выбрать", key=_k("sel", item), value=(str(item) in st.session_state["selected_dirs"]), help="В очередь", label_visibility="collapsed")
                        if checked:
                            st.session_state["selected_dirs"].add(str(item))
                        else:
                            st.session_state["selected_dirs"].discard(str(item))
                with name_cols[2]:
                    if st.button("✏️", key=_k("ren", item), help="Переименовать", use_container_width=True):
                        st.session_state["rename_target"] = str(item)
                with name_cols[3]:
                    if st.button("🗑️", key=_k("del", item), help="Удалить", use_container_width=True):
            ...
            if st.session_state.get("rename_target") == str(item):
                ...
                with rc1:
                    new_name_val = st.text_input("Новое имя", value=item.name, key=_k("ren_input", item), label_visibility="collapsed")
                with rc2:
                    if st.button("Сохранить", key=_k("save", item), use_container_width=True):
                        try:
                            ...
                            st.error(f"Ошибка: {e}")
                with rc3:
                    if st.button("Отмена", key=_k("cancel", item), use_container_width=True):
            ...
            if st.session_state.get("delete_target") == str(item):
                ...
                with dc1:
                    ...
                    if send2trash is None:
                        ...
                        confirm_text = st.text_input("Введите имя для подтверждения", key=_k("confirm_name", item))
                ...
                with dc2:
                    ...
                    if st.button("Удалить", type="primary", key=_k("confirm_del", item), use_container_width=True, disabled=not can_delete):
                        try:
                ...
                with dc3:
                    if st.button("Отмена", key=_k("cancel_del", item), use_container_width=True):
...
st.title("Face Sorter — обложки папок, все объекты, DnD файлов")
...
if st.session_state["parent_path"] is None:
    ...
    with st.container(height=780):
        # DnD block (per-file reorder and move to any folder)
        render_dnd_files_any_folder(curr, parent_root, max_depth=5, max_nodes=400)
        st.markdown("---")

        # Explorer grid (all objects; folders with covers; obey session order for files)
        render_explorer_grid(curr, parent_root)
