# app.py
# Minimal Explorer (revised++): LAN URL, cached 150x150 thumbnails, richer logs, safe delete/copy,
# atomic JSON writes, UI fixes, and root-safe "Up" logic.
...
def load_index(parent: Path) -> Dict:
    ensure_dir(parent)  # гарантируем существование каталога
...
def save_index(parent: Path, idx: Dict):
    ensure_dir(parent)  # фикс FileNotFoundError
...
# ---- Network URL helpers ----
def get_lan_ip() -> str:
    """Надёжно определяет LAN-IP активного интерфейса."""
    try:
...
def get_network_url() -> str:
    """Собирает сетевой URL вида http://<lan_ip>:<port>/<baseUrlPath>."""
    ip = get_lan_ip()
...
def _init_state():
    ...
    st.session_state.setdefault("delete_target", None)   # str path for confirm delete
...
def human_size(n: int) -> str:
    """Форматирование размера файла (байты → ТБ)."""
...
def cleanup_processed_images(group_dir: Path, processed_images: Set[Path]):
    # Не удаляем файлы внутри целевых директорий назначения
...
def match_and_apply(group_dir: Path, plan: Dict, match_thr: float) -> Tuple[int, Set[Path]]:
    ...
    # --- перенос изображений по кластерам (безопасное копирование) ---
...
def get_thumb_bytes(path_str: str, size: int, mtime: float):
    """Кэшируем миниатюру по пути и времени модификации."""
    if Image is None:
        return None
...
if st.session_state["parent_path"] is None:
    ...
    with top_cols[0]:
        # корректная логика "Вверх" на корне
    ...
    for item in items:
        ...
        # Preview (150x150 for images only, cached)
        with c1:
    ...
    if st.button("▶️ Обработать", type="primary", use_container_width=True):
        ...
        if not targets:
            ...
            st.session_state["queue"] = []
            st.session_state["selected_dirs"] = set()  # сброс чекбоксов после прогона
