...
def load_config(base: Path) -> Dict:
    ...
    defaults = {
        ...
        "ultra_weights": "yolov8n-face.pt",  # имя модели/URL/путь для автозагрузки
...
def load_yolo_ultra(weights_path: str, device: str = "cpu"):
    """
    Загружает модель Ultralytics YOLO. Поддерживаются:
    - локальный путь к .pt/.onnx;
    - имя предобученной модели (например, "yolov8n.pt" или комьюнити-"yolov8n-face.pt");
    - прямой URL до веса — Ultralytics сам скачает и закеширует.
    Если пакет ultralytics не установлен — возвращает None.
    Устройство задаётся на этапе predict(), а здесь только инициализация.
    """
    try:
        ...
        return None
    try:
        # Не проверяем наличие файла: YOLO сам скачает по имени/URL.
