# app.py — Face Sorter (per-file DnD, move-to-any-folder)
# Features:
# - Перетаскивание КАЖДОГО файла: порядок внутри текущей папки (UI порядок сохраняется в сессии)
# - Перетаскивание файла на любую папку под root ⇒ физический move с коллизиями имён
# - Проводник, переименование/удаление, очередь групп, обработка c build_plan
# - Безопасные операции, мини-аудит, LAN URL picker
...
st.set_page_config(page_title="Face Sorter — перетаскивание файлов", layout="wide")
            ...
            for src_str in cont.get('items', []):
                ...
                try:
                    ...
                    if src.parent.resolve() == dst_dir.resolve():
                        skp += 1; continue
                    # no self-subtree check needed for files, keep parity anyway
                    if _is_subpath(dst_dir, src):
                        skp += 1; continue
...
st.title("Face Sorter — перетаскивание файлов")
...
if st.session_state["parent_path"] is None:
    ...
    with st.container(height=760):
        ...
        # Explorer grid (respect session order)
        items = apply_session_order(curr, list_dir(curr))
        for item in items:
            is_dir = item.is_dir()
            sel_key = _k("sel", item)
