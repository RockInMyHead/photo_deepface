...
# LAN URL, кэш превью, безопасные операции с файлами, атомарная запись JSON,
# скролл-проводник, DnD-перемещение, предобработка: YOLO→кроп лица→кластеризация (fallback на Haar).
...
def load_config(base: Path) -> Dict:
    p = base / "config.json"
    defaults = {
        # кластеризация/матчинг (ослаблены → выше recall)
        "group_thr": 3,
        "eps_sim": 0.50,
        "min_samples": 2,
        "min_face": 80,
        ...
        "ultra_weights": "models/face.pt",  # yolov5/8-face .pt
...
def load_yolo_ultra(weights_path: str, device: str = "cpu"):
    """
    Загружает модель Ultralytics YOLO из .pt. Возвращает None, если
    пакет не установлен или файл не найден.
    """
    try:
    ...
    try:
        model = YOLO(str(wp))
        # model.to(device) — обычно не обязательно, device указывается в predict()
        return model
    except Exception:
        return None
...
def detect_faces_ultra(model, image_path: Path, conf_thr: float, iou_thr: float, imgsz: int, device: str) -> List[Tuple[int,int,int,int]]:
    """
    Детект лиц через Ultralytics. Возвращает список bbox (x1,y1,x2,y2).
    """
    if model is None:
        return []
    try:
...
def detect_faces_haar(image_path: Path) -> List[Tuple[int,int,int,int]]:
    """
    Fallback: классический Haar-каскад OpenCV.
    """
    if cv2 is None:
        return []
    try:
...
def crop_faces_with_ultra(source_dir: Path, temp_dir: Path, model, conf_thr: float, iou_thr: float, imgsz: int, device: str):
    """
    Пробегает по картинкам, детектит лица Ultralytics (fallback — Haar),
    сохраняет кропы лиц в temp_dir. Если лицо не найдено — копирует оригинал.
    """
    ensure_dir(temp_dir)
    for ext in IMG_EXTS:
...
if st.session_state["parent_path"] is None:
    ...
    with st.container(height=700):
        ...
        for item in items:
            ...
            with c1:
                if not is_dir and item.suffix.lower() in IMG_EXTS:
                    try:
                        ...
                        data = None
                    if data: st.image(data)
                    else: st.image(str(item), width=150)
                else:
            ...
            with c2:
                ...
                with name_cols[1]:
                    if is_dir:
                        ...
                        if checked: st.session_state["selected_dirs"].add(str(item))
                        else: st.session_state["selected_dirs"].discard(str(item))
                with name_cols[2]:
            ...
            with c4:
                try: st.write(datetime.fromtimestamp(item.stat().st_mtime).strftime("%Y-%m-%d %H:%M"))
                except Exception: st.write("—")
            with c5:
                if is_dir: st.write("—")
                else:
                    try: st.write(human_size(item.stat().st_size))
                    except Exception: st.write("—")
                ...
                with rc2:
                    if st.button("Сохранить", key=f"save::{item}", use_container_width=True):
                        try:
                            ...
                            if new_path.exists(): st.error("Файл/папка с таким именем уже существует.")
                            else:
                                item.rename(new_path); st.session_state["rename_target"] = None; st.rerun()
                        except Exception as e:
                            st.error(f"Ошибка: {e}")
                with rc3:
            ...
            if st.session_state.get("delete_target") == str(item):
                ...
                with dc1: st.markdown(f"❗ Подтвердите удаление: **{item.name}**")
                with dc2:
                    if st.button("Удалить", type="primary", key=f"confirm_del::{item}", use_container_width=True):
                        try:
                            if send2trash is not None:
                                ...
                                shutil.rmtree(item, ignore_errors=True) if is_dir else item.unlink(missing_ok=True)
    ...
    with st.expander("Переместить файлы (Drag & Drop)", expanded=False):
        if sort_items is None:
            ...
            for f in subfolders: containers.append({"header": f.name, "items": []})
            ...
            if st.button("Применить переносы", use_container_width=True):
                ...
                for src, dst_dir in moves:
                    try: safe_move(src, dst_dir); ok += 1
                    except Exception as e:
                        errors += 1; st.error(f"Не удалось переместить {src.name}: {e}")
    ...
    if st.button("▶️ Обработать", type="primary", use_container_width=True):
        ...
        if not targets: targets = [p for p in curr.iterdir() if p.is_dir()]
        if not targets:
            ...
            # preload Ultralytics
            ultra_model = load_yolo_ultra(CFG.get("ultra_weights"), CFG.get("ultra_device", "cpu")) if CFG.get("use_ultralytics", False) else None
            st.caption(f"ULTRA enabled={CFG.get('use_ultralytics', False)} loaded={ultra_model is not None} weights={CFG.get('ultra_weights')} device={CFG.get('ultra_device')}")
